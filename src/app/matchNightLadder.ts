...
export const resolveMatchNights = (guildId: string, configured: MatchNight[]): MatchNight[] => {
  if (configured.length > 0)
    return [...configured].sort((a, b) => a.preferenceOrder - b.preferenceOrder)
  return DEFAULT_MATCH_NIGHTS.map((n) => ({ ...n, guildId }))
}

export const findNightByOrder = (nights: MatchNight[], order: number): MatchNight | null =>
  nights.find((n) => n.preferenceOrder === order) ?? null
